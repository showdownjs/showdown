/**
 * Unit tests for the `cmSpec` option (CommonMark autolinks).
 * The behavior is gated: it is off by default and enabled by the `commonmark` flavor.
 */

describe('showdown.Converter cmSpec option (Autolinks)', function () {
  'use strict';

  describe('disabled (default)', function () {
    let converter = new showdown.Converter();

    it('should entity-encode email autolinks (Showdown default)', function () {
      expect(converter.makeHtml('<foo@bar.example.com>')).toMatch(/&#1\d\d;/);
    });
  });

  describe('enabled', function () {
    // encodeEmails now works under cmSpec, so disable it here to assert the plain,
    // spec-compliant mailto form (the commonmark flavor disables it for the same reason)
    let converter = new showdown.Converter({cmSpec: true, encodeEmails: false});

    it('should autolink arbitrary URI schemes', function () {
      expect(converter.makeHtml('<irc://foo.bar:2233/baz>')
      ).toBe('<p><a href="irc://foo.bar:2233/baz">irc://foo.bar:2233/baz</a></p>');
    });

    it('should autolink made-up schemes', function () {
      expect(converter.makeHtml('<made-up-scheme://foo,bar>')
      ).toBe('<p><a href="made-up-scheme://foo,bar">made-up-scheme://foo,bar</a></p>');
    });

    it('should autolink emails with a plain mailto: (no entity encoding)', function () {
      expect(converter.makeHtml('<foo@bar.example.com>')
      ).toBe('<p><a href="mailto:foo@bar.example.com">foo@bar.example.com</a></p>');
    });

    it('should escape ampersands in the autolink href and text', function () {
      expect(converter.makeHtml('<http://foo.bar.baz/test?q=hello&id=22&boolean>')
      ).toBe('<p><a href="http://foo.bar.baz/test?q=hello&amp;id=22&amp;boolean">' +
                      'http://foo.bar.baz/test?q=hello&amp;id=22&amp;boolean</a></p>');
    });
  });

  describe('enabled via the commonmark flavor', function () {
    let converter = new showdown.Converter(showdown.getFlavorOptions('commonmark'));

    it('should autolink a uppercase MAILTO scheme as a URI', function () {
      expect(converter.makeHtml('<MAILTO:FOO@BAR.BAZ>')
      ).toBe('<p><a href="MAILTO:FOO@BAR.BAZ">MAILTO:FOO@BAR.BAZ</a></p>');
    });
  });
});
